...
def load_buildings(json_path):
    ...
    Returns: list of building dict records (each record should have xmin/xmax/ymin/ymax/zmin/zmax;
             may additionally have "footprint": [[x,y], ...]).
    """
    ...
    if isinstance(data, dict):
        ...
        if "boxes" in data and isinstance(data["boxes"], list):
            return data["boxes"]
        # If dict but no known key, return empty to fail safe
        return []
...
class Nav3DEnv(EnvBase):
    def __init__(
        ...
        **kwargs,
        ...
        # buildings -> bounds tensor (+ optional footprint polygon tensors) on device
        ...
        if len(self.buildings) > 0:
            ...
            for b in self.buildings:
                ...
                bounds.append([xmin, ymin, zmin, xmax, ymax, zmax])
                xs += [xmin, xmax]
                ys += [ymin, ymax]

                # optional footprint; fallback to AABB rectangle footprint if missing/malformed
                fp = b.get("footprint", None)
                if isinstance(fp, list) and len(fp) >= 3:
                    fp_xy = np.asarray(fp, dtype=np.float32)
                    if fp_xy.ndim == 2 and fp_xy.shape[1] == 2 and np.isfinite(fp_xy).all():
                        footprints.append(fp_xy)
                        fp_lens.append(fp_xy.shape[0])
                    else:
                        footprints.append(np.asarray([[xmin, ymin], [xmax, ymin], [xmax, ymax], [xmin, ymax]], dtype=np.float32))
                        fp_lens.append(4)
                else:
                    footprints.append(np.asarray([[xmin, ymin], [xmax, ymin], [xmax, ymax], [xmin, ymax]], dtype=np.float32))
                    fp_lens.append(4)
            ...
            # footprints padded tensors
            self.has_footprints = True
            M = len(footprints)
            Vmax = int(max(fp_lens)) if M > 0 else 0
            self.footprint_len = torch.tensor(fp_lens, dtype=torch.long, device=self.device)  # [M]

            fp_pad = torch.full((M, Vmax, 2), float("nan"), dtype=torch.float32, device=self.device)
            fp_mask = torch.zeros((M, Vmax), dtype=torch.bool, device=self.device)

            for i, fp_xy in enumerate(footprints):
                v = fp_xy.shape[0]
                if v <= 0:
                    continue
                fp_pad[i, :v, 0] = torch.as_tensor(fp_xy[:, 0], dtype=torch.float32, device=self.device)
                fp_pad[i, :v, 1] = torch.as_tensor(fp_xy[:, 1], dtype=torch.float32, device=self.device)
                fp_mask[i, :v] = True

            self.footprint_xy = fp_pad          # [M,Vmax,2]
            self.footprint_mask = fp_mask       # [M,Vmax]
            ...
            self.has_footprints = False
            self.footprint_xy = None
            self.footprint_mask = None
            self.footprint_len = None
    ...
    def _inside_any_building(self, pos: torch.Tensor) -> torch.Tensor:
        """
        Check whether each point is inside ANY building volume proxy.

        Improved logic:
          1) AABB broad-phase (fast, vectorized)
          2) z-range gate (already included in AABB)
          3) XY point-in-polygon against footprint (ray casting), also vectorized
             - If footprints missing, falls back to AABB-only.
        ...
        B, K = int(pos.shape[0]), int(pos.shape[1])
        ...
        aabb_hit = (in_x & in_y & in_z)  # [B,K,M]

        # If no footprints available, keep legacy behavior: AABB-only
        if (not getattr(self, "has_footprints", False)) or self.footprint_xy is None or self.footprint_mask is None:
            return aabb_hit.any(dim=-1)

        # Vectorized point-in-polygon (ray casting) for footprints
        # footprint_xy: [M,V,2], mask: [M,V]
        fp = self.footprint_xy  # [M,V,2]
        msk = self.footprint_mask  # [M,V]
        M, V = int(fp.shape[0]), int(fp.shape[1])

        # Prepare polygon vertices and rolled (next) vertices
        xi = fp[:, :, 0]  # [M,V]
        yi = fp[:, :, 1]  # [M,V]
        xj = torch.roll(xi, shifts=-1, dims=1)  # [M,V]
        yj = torch.roll(yi, shifts=-1, dims=1)  # [M,V]

        mi = msk  # [M,V]
        mj = torch.roll(msk, shifts=-1, dims=1)  # [M,V]
        valid_edge = (mi & mj)  # [M,V]

        # Broadcast to [B,K,M,V]
        px = pos[:, :, 0].unsqueeze(2).unsqueeze(3)  # [B,K,1,1]
        py = pos[:, :, 1].unsqueeze(2).unsqueeze(3)  # [B,K,1,1]

        xi_b = xi.view(1, 1, M, V)
        yi_b = yi.view(1, 1, M, V)
        xj_b = xj.view(1, 1, M, V)
        yj_b = yj.view(1, 1, M, V)
        ve_b = valid_edge.view(1, 1, M, V)

        # Edge crosses horizontal ray?
        cond_y = (yi_b > py) != (yj_b > py)  # [B,K,M,V]
        # intersection x coordinate
        denom = (yj_b - yi_b)
        denom = torch.where(denom.abs() < 1e-12, torch.full_like(denom, 1e-12), denom)
        xinters = (xj_b - xi_b) * (py - yi_b) / denom + xi_b  # [B,K,M,V]

        cond_x = xinters > px
        crossing = cond_y & cond_x & ve_b  # [B,K,M,V]

        crossings = crossing.sum(dim=-1)  # [B,K,M]
        inside_poly = (crossings % 2 == 1)  # [B,K,M]

        inside = (aabb_hit & inside_poly).any(dim=-1)  # [B,K]
        return inside
    ...
    def get_uav_positions(self):
        # shape: [B, N, 3]  (B=num_envs batch)
        return None if self.uav_pos is None else self.uav_pos.detach().clone()
    
    def get_target_positions(self):
        return None if self.targets is None else self.targets.detach().clone()

    def set_eval_case(self, uav_pos, targets, env_idx: int = 0):
        """
        Store a scenario that will be applied on next reset.
        uav_pos: (N,3) or (B,N,3)
        targets: (N,3) or (B,N,3)
        """
        import numpy as np
        import torch
    
        uav_pos = torch.as_tensor(uav_pos, dtype=torch.float32, device=self.device)
        targets = torch.as_tensor(targets, dtype=torch.float32, device=self.device)
    
        # normalize to batched [B,N,3]
        if uav_pos.ndim == 2:
            uav_pos = uav_pos.unsqueeze(0).expand(self.num_envs, -1, -1)
        if targets.ndim == 2:
            targets = targets.unsqueeze(0).expand(self.num_envs, -1, -1)
